...
class Growtype_Form_Google
{
    ...
    function current_request_is_excluded()
    {
        return apply_filters('growtype_form_google_auth_request_is_excluded', false, $_SERVER['REQUEST_URI'] ?? '');
    }

    function validate_response()
    {
        if (isset($_GET['code'])) {
            $request_is_excluded = self::current_request_is_excluded();

            if (!$request_is_excluded) {
                $client = $this->init_client();

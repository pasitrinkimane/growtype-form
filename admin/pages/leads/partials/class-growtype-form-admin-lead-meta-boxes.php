...
class Growtype_Form_Admin_Lead_Meta_Boxes
{
    ...
    /**
     * Register gfemail fields in the lead edit meta boxes.
     */
    public static function add_lead_meta_fields(array $meta_boxes): array
    {
        $fields = [
            ["title" => "Source", "key" => "source"],
            ["title" => "Quiz Token", "key" => "gqtoken"],
            ["title" => "IP Address", "key" => "ip"],
            ["title" => "User Agent", "key" => "user_agent"],
        ];

        $marketing_keys = Growtype_Form_Email_Page::marketing_keys();
        ...
        $meta_boxes[0]["fields"] = array_merge(
            $meta_boxes[0]["fields"] ?? [],
            $fields,
        );
    ...
    public function __construct()
    {
        ...
        // Register gfemail extra details in lead edit screen
        add_filter(
            "growtype_form_lead_meta_boxes",
            [self::class, "add_lead_meta_fields"],
            100,
        );
         ...
         * Admin footer
         */
        add_action("admin_footer", [$this, "admin_footer_extend"]);
    }
    ...
    public function render_custom_meta_box($post, $params)
    {
        ...
        <div style="display: flex; gap: 10px; flex-direction: column;">
            <?php foreach ($params["args"]["fields"] as $field) {

                $meta_value = get_post_meta($post->ID, $field["key"], true);
                $type = $field["type"] ?? "text";

                // Skip empty fields (except special types)
                if (
                ...
                <div style="display: flex; gap: 10px; align-items: center;">
                    <label style="min-width: 150px; max-width: 150px;" for="<?php echo esc_attr(
                        $field["key"],
                    ); ?>">
                                    ...
                                    <?php else: ?>
                                        <?php foreach (
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                        <span style="color: <?php echo $success
                                                            ? "#2271b1"
                                                            : "#d63638"; ?>; font-weight: bold;">
                                                            <?php echo $success
                                                                ? "✓"
                                                                : "✗"; ?>
                                                        </span>
                                ...
                                jQuery(document).ready(function ($) {
                                    $('#toggle-events-log-view').off('click').on('click', function () {
                                        ...
                                        if (!isTableVisible) {
                                            ...
                                            var editorElement = $('#<?php echo esc_js(
                                                $field["key"],
                                            ); ?>');
    ...
    public function save_custom_meta_boxes($post_id)
    {
        ...
        if (
            get_post_type($post_id) !== Growtype_Form_Admin_Lead::POST_TYPE_NAME
        ) {
            return;
        }
